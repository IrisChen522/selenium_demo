from common.enum_element import Element
...
def get_error(kw):
    elements = {
        ...
        Element.CONTRARY_GETVAL: lambda: "==检查点_对比数据失败，当前取到到数据为:%s,目标数据为:%s" % (kw["current"], kw["target"]),
        Element.DEFAULT_CHECK: lambda: "==检查点_%s失败，请检查_%s==" % (kw["info"], kw["element_info"]),
        Element.COMPARE: lambda: "==检查点_对比数据失败，当前取到到数据为:%s,目标数据为:%s" % (kw["current"], kw["target"]),
